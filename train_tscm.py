...
transform = transforms.Compose([
    ...
    transforms.RandomResizedCrop(size=1024, scale=(0.9, 1.11), ratio=(0.9, 1.11)),  
...
for epoch in range(1, num_epochs + 1):
    ...
    for x in progress_bar:
        ...
        # x_compr = hue_transform(x_compr)
        x_recon, vq_loss, perplexity, _ = model(x_compr)
        mse_loss = F.mse_loss(x_recon, x)
        ssim_loss = 1 - ssim(x_recon, x, data_range=1, size_average=True)
        loss = mse_loss + 0.05*ssim_loss + 0.02*vq_loss
        loss.backward()
        optimizer.step()
